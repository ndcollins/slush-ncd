...
module.exports = function(gulp) {
    ...
    gulp.task('route', function(done) {
        ...
        if (modules.length === 0) {
          throw new Error('route must be created in a module, but no modules exist. Create a module using "slush angular-gulp:module <module-Name>".');
        }
        ...
        inquirer.prompt([{
            ...
            //Source
            gulp.src(__dirname + '/../templates/route/route.js')
                .pipe(template(answers))
                .pipe(rename(answers.fileName + '-route.js'))
